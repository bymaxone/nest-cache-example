...
> **Document version:** 1.0 — initial dashboard blueprint. Status: specification only (the `apps/web` app and the `apps/api` admin/metrics/events modules are not yet built). This file is the build base for `apps/web`; see [`TECHNICAL_SPECIFICATION.md` §25](TECHNICAL_SPECIFICATION.md#25--phased-delivery-plan) for the suggested build order (Phases 6–7 cover the dashboard). Maintained by **Bymax One** · MIT.
