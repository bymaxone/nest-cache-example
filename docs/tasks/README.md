...
> **Quality bar — full library-grade (revised).** This reference app is copied as a template and must prove the consumed `@bymax-one/nest-cache` surface exhaustively before publish, so it adopts the **siblings' bar**: **100% unit coverage** (api Jest + web Vitest), **E2E of every HTTP + WebSocket flow** (Testcontainers), and **Stryker mutation** — **api `break: 100`** (achieved **100.00%**, 0 survivors) / **web `break: 90`** (achieved **91.61%**, `lib/**` 100%) — on top of the Phase 15 Testcontainers smoke, the web build + Playwright journey suite, and the export-usage audit. This **supersedes** the original lighter "example-app smoke" bar (and the spec's NG4). See [`DEVELOPMENT_PLAN.md` Appendix C](../DEVELOPMENT_PLAN.md#appendix-c--quality-gates).
