...
- P1-3 (post-review fix) ✅ 2026-06-16 — Applied security/code-review findings: added `--cluster-announce-ip 127.0.0.1` to all 6 cluster nodes, added healthchecks to all 6 cluster nodes and `redis-master`, added `depends_on: condition: service_healthy` to sentinels and replicas, moved sentinel.conf to named volumes (prevents dirty working tree on failover), pinned `redis/redisinsight:2` image, scrubbed phase-stage comment refs.
